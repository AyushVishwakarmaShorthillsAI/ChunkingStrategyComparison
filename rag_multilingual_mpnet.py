...
COLLECTION_NAME = "Multilingual_MPNet_Collection"
...
def ingest_data_mpnet(chunks, model_name, collection_name):
...
def query_rag_mpnet(query, model_name, collection_name):
...
def save_results(query, results, model_name):
    ...
    filename = f"query_multilingual_mpnet_results.json"
...
if __name__ == "__main__":
    ...
    if chunks:
        # 2. Ingest with MPNet embeddings
        success = ingest_data_mpnet(chunks, MODEL_NAME, COLLECTION_NAME)
        
        if success:
            # 3. Query
            results = query_rag_mpnet(QUERY, MODEL_NAME, COLLECTION_NAME)
