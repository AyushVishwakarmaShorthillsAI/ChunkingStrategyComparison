...
COLLECTION_NAME = "GTE_Multilingual_Collection"
...
def ingest_data_gte(chunks, model_name, collection_name):
    ...
    try:
        ...
        # GTE Multilingual usually requires trust_remote_code for some versions or specific configs
...
def query_rag_gte(query, model_name, collection_name):
...
def save_results(query, results, model_name):
    ...
    filename = f"query_gte_results.json"
...
if __name__ == "__main__":
    ...
    if chunks:
        # 2. Ingest with GTE embeddings
        success = ingest_data_gte(chunks, MODEL_NAME, COLLECTION_NAME)
        
        if success:
            # 3. Query
            results = query_rag_gte(QUERY, MODEL_NAME, COLLECTION_NAME)
