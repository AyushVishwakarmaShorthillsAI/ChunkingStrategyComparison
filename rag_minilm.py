...
def ingest_data_minilm(chunks, model_name, collection_name):
...
def query_rag_minilm(query, model_name, collection_name):
...
def save_results(query, results, model_name):
    ...
    filename = f"query_minilm_results.json"
...
if __name__ == "__main__":
    ...
    if chunks:
        # 2. Ingest with MiniLM embeddings
        success = ingest_data_minilm(chunks, MODEL_NAME, COLLECTION_NAME)
        
        if success:
            # 3. Query
            results = query_rag_minilm(QUERY, MODEL_NAME, COLLECTION_NAME)
