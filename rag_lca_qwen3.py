...
MODEL_NAME = "BIaoo/lca-qwen3-embedding"
COLLECTION_NAME = "LCA_Qwen3_Collection"
...
def ingest_data_lca(chunks, model_name, collection_name):
...
def query_rag_lca(query, model_name, collection_name):
...
def save_results(query, results, model_name):
    ...
    filename = f"query_lca_qwen3_results.json"
...
if __name__ == "__main__":
    ...
    if chunks:
        # 2. Ingest with LCA-Qwen3 embeddings
        success = ingest_data_lca(chunks, MODEL_NAME, COLLECTION_NAME)
        
        if success:
            # 3. Query
            results = query_rag_lca(QUERY, MODEL_NAME, COLLECTION_NAME)
